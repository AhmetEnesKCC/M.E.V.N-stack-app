    ...
    <v-main>
      ...
      <v-card class="mt-10">
        <v-tabs v-model="tabType" color="success" grow>
          <v-tab v-for="tab in tabsTitles" :key="tab.id">{{ tab.text }}
          ...
          <v-tabs-items v-model="tabType">
                  ...
                  <v-list-item v-for="todo in todos" :key="todo.id">
                    ...
                    <v-list-item-content>
                      <div class="d-flex flex-row align-center">
                        <v-checkbox :input-value="todo.completed" v-on:click="updateCompleted(todo)" class="ml-5 mr-5"
                                    color="success"
                        ></v-checkbox>
            ...
            <v-tab-item>
              <v-text-field  class="pa-5" v-model="input" placeholder="Please Enter your Todo"
                            color="success"></v-text-field>
              <v-btn style="width: 80%;margin-left: 10%" class="align-center" v-on:click="addTodo">Add</v-btn>
            </v-tab-item>
            <v-tab-item>
              <v-btn @click="log(index)">Test</v-btn>
              <v-card color="success">

                <v-list>
                  <v-list-item v-for="todo in todos" :key="todo._id">
                    <v-list-item-content>
                      <v-text-field color="success" v-model="index[todo._id]"
                                    :placeholder="todo.todo"></v-text-field>
                    </v-list-item-content>
                    <v-list-item-action class="d-flex flex-row align-center">
                      <v-btn v-on:click="updateTodo(todo)" class="ml-3 mr-3" icon>
